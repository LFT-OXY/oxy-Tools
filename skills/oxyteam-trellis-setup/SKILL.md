...
洞 U  禁令没有解除条件，管不着的回合上只能被违反（v0.4.21 修，三个块补出路）
      同版复测：挂着 flow_stage=discover 的任务（「我的预订」页面），
      用户紧接着要一件无关的活（写 .editorconfig）。模型的第一句话是
      「这一轮跟 my-bookings-page 无关，不走它的阶段约束」，然后照常
      写文件 —— 对照 S-0 的原始现场：同样的处境、同样的活，那次是
      一声不吭直接写了 47 行。行为都是「写了文件」，性质从「禁令失效」
      变成「禁令按设计让路」，而且判断被说了出来

洞 V  挡位跳跃没有准入条件（v0.4.21 修，两处切挡位前补准入）
      同版复测，且比原始现场更严格 —— S-1 是模型自己决定跳挡位，这次是
      用户明确要求「直接进 specify」。模型明说「进 specify 的准入没过」,
      把 4 条自己提过而用户没回答的问题一条条列出来，「所以我不往下走」,
      并给了两条出路（一次答完 / 明说这几条不用管由它拍板）。
      复测后 task.json 的 flow_stage 仍是 discover，没被切走
```
