...
**洞 AA 同一次改的两段，只有第一段有等价观测，第二段没有** —— 复测那轮模型报了
「除当前任务外没有别的活跃任务」（对应「别顺手归档别的 `in_progress` 任务」），
但那是单会话场景，触发不到它真正要防的风险；第二段要求数的 `sessions/` 计数
一个字没提，归档又会清掉会话文件，事后查不出它数没数。**所以洞 AA 不进上面的
列表**，v0.4.19 把那句措辞改成「把数出来的数字说出来」，等双会话场景跑过再补。
这一条本身就是记录块要防的东西的活样本：**修了两段，其中一段的「已验」是错觉。**
