---
name: oxyteam-init
description: "为 Oxyteam 工程 Skills 配置当前代码库：设置问题跟踪器、分诊标签词汇和领域文档布局。首次使用其他工程 Skills 前运行一次。"
...
Lead each section with the recommended answer so the user can accept it in a word. Give a one-line explainer only when the choice genuinely branches; skip the section entirely when exploration already settled it (Section B when `oxyteam-triage` isn't installed, Section C when there's no monorepo).
...
**Section B — Triage label vocabulary.** Skip this section entirely if the `oxyteam-triage` skill isn't installed (exploration told you) — an uninstalled skill needs no labels.

If it is installed, ask exactly one question:

> Do you want to keep the default triage labels? (recommended: **yes**)

The defaults are the five canonical roles, each label string equal to its name: `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`. On **yes**, write them as-is. Only if the user says no — usually because their tracker already uses other names (e.g. `bug:triage` for `needs-triage`) — collect the overrides so `oxyteam-triage` applies existing labels instead of creating duplicates.
...
- The contents of `docs/agents/issue-tracker.md`, `docs/agents/domain.md`, and `docs/agents/triage-labels.md` (the last only when `oxyteam-triage` is installed)
...
Include the `### Triage labels` sub-block, and write `docs/agents/triage-labels.md`, only when `oxyteam-triage` is installed and Section B ran. When it isn't, both are omitted.
...
- [triage-labels.md](./triage-labels.md) — label mapping (only if `oxyteam-triage` is installed)
